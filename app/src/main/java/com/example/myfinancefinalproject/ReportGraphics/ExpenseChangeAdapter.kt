...
class ExpenseChangesAdapter : RecyclerView.Adapter<ExpenseChangesAdapter.VH>() {

    private val items = mutableListOf<ExpenseChangeUi>()
    private val sdf = SimpleDateFormat("dd MMM, HH:mm", Locale("ru"))

    fun submit(list: List<ExpenseChangeUi>) {
        items.clear()
        items.addAll(list)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VH {
        val v = LayoutInflater.from(parent.context)
            .inflate(R.layout.item_expense_change, parent, false)
        return VH(v)
    }
